...
21. PWA 세팅해서 앱으로 발행하기(모바일 앱처럼 꾸미기)

#################################
PWA(Progressive Web App):
웹사이트를 네이트브 앱처럼 사용할 수 있도록 하는 웹개발 기술.
ios, aoi 앱으로 발행하는 게 아니라 웹 사이트 자체를 스마트폰 홈 화면에 설치하는 것이다.
...
pwa를 만드려면 pwa가 세팅된 리액트 프로젝트가 필요하다

npx create-react-app 프로젝트명 --template cra-t
emplate-pwa

반드시 처음부터 pwa로 만들어진 프로젝트가 필요하다.
기존 프로젝트는 안 된다. 따라서 pwa 만들고 기존 프로젝트 코드 복붙.

pwa의 조건
1. manifest.json 이 있어야 한다.
2. service-work.js가 있어야 한다.

pwa 프로젝트로 만들면 다 만들어 진다.

manifest.json 은 public 디렉토리 내에 있다. 이게 앱 정보이다.

service-worker.js를 만드려면 index.js에서
serviceworkerRegistration.unregister를 -> register로 바꾸면 된다.
물론 프로젝트 생성 직후에도 존재하지만 그건 설정파일이라고 보면 되고,
여기에서 실제 파일은 것은 빌드할 때 생성된다.
이 파일은 오프라인에서도 열수 있게 도와준다.

빌드하면 build 폴더가 생성되면서 그 안에
위 두 가지 파일이 들어가게 된다.
asset-manifest.json은 하드에 캐싱할 목록이 들어있다.

특정 파일들을 캐싱하지 못하도록 하려면
node_modules/react-scripts/config/webpack.config.js

여기서 InjectManifest 부분에서 exclude 항목에 정규식으로 추가해줘야 한다.
